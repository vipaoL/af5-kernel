...
extern void sci_ctrl_pmu_sleep_ctrl(u32 flag);
extern int sci_get_couter4pmu_apb_sleep_ctrl();
...
int sr2351_fm_en(void)
{
	...
	if(sci_get_couter4pmu_apb_sleep_ctrl() < 2) {
		sci_ctrl_pmu_sleep_ctrl(1);
	}

	SR2351_PRINT("couter4pmu_apb_sleep_ctrl in sr2351_fm_en : %d\r\n", sci_get_couter4pmu_apb_sleep_ctrl());
...
int sr2351_fm_dis(void)
{
	...
	sci_ctrl_pmu_sleep_ctrl(0);
	SR2351_PRINT("couter4pmu_apb_sleep_ctrl in sr2351_fm_dis : %d\r\n", sci_get_couter4pmu_apb_sleep_ctrl());
...
void sr2351_fm_enter_sleep(void)
{
    ...
    read_fm_reg(FM_REG_AGC_TBL_STS,&reg_data);
    SR2351_PRINT("value of FM_REG_AGC_TBL_STS is %08x\r\n",reg_data);
    gain_data = reg_data & 0x0F;
    SR2351_PRINT("gain_data in FM_REG_AGC_TBL_STS is %08x\r\n",gain_data);
	...
	if(fm_rf_ops != NULL)
	{
        ...
        read_fm_reg(FM_REG_AGC_CTRL,&reg_data);
        SR2351_PRINT("value of FM_REG_AGC_CTRL before is %08x\r\n",reg_data);
        reg_data &= ~(0xF<<16);
        reg_data |= (gain_data<<16);
        sci_glb_write(FM_REG_AGC_CTRL,reg_data,-1UL);
        read_fm_reg(FM_REG_AGC_CTRL,&reg_data);
        SR2351_PRINT("value of FM_REG_AGC_CTRL after is %08x\r\n",reg_data);
