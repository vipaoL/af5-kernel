...
LOCAL unsigned int _sensor_k_get_id_pin(void)
{
	unsigned int pin_status = 0;

	switch (_sensor_K_get_curId()) {
		case SENSOR_MAIN:
			gpio_direction_input(s_p_sensor_mod->pin_main_id);
			pin_status = (gpio_get_value(s_p_sensor_mod->pin_main_id) ? 1 : 0);
			printk("_sensor_k_get_id_pin: main id pin_status=%d \n", pin_status);
			break;
		case SENSOR_SUB:
			gpio_direction_input(s_p_sensor_mod->pin_sub_id);
			pin_status = (gpio_get_value(s_p_sensor_mod->pin_sub_id) ? 1 : 0);
			printk("_sensor_k_get_id_pin: sub id pin_status=%d \n", pin_status);
			break;
		default:
			break;
	}
				...
				unsigned long arg)
{
	...
	case SENSOR_IO_DETECT_ID_PIN:
		{
			unsigned int id_pin;

			id_pin = _sensor_k_get_id_pin();
			ret = copy_to_user((unsigned int *)arg, &id_pin, sizeof(unsigned int));
		}
		break;
...
int sensor_k_probe(struct platform_device *pdev)
{
	...
	s_p_sensor_mod->pin_main_id = of_get_gpio(sensor_dev.this_device->of_node,4);
	s_p_sensor_mod->pin_sub_id = of_get_gpio(sensor_dev.this_device->of_node,5);
#else
	...
	printk("sensor pin_main_id =%d\n",s_p_sensor_mod->pin_main_id);
	printk("sensor pin_sub_id =%d\n",s_p_sensor_mod->pin_sub_id);
	...
	ret = gpio_request(s_p_sensor_mod->pin_main_id, "main camera id");
	if (ret) {
		tmp = s_p_sensor_mod->pin_main_id;
		goto gpio_err_exit;
	}

	ret = gpio_request(s_p_sensor_mod->pin_sub_id, "sub camera id");
	if (ret) {
		tmp = s_p_sensor_mod->pin_sub_id;
		goto gpio_err_exit;
	}
