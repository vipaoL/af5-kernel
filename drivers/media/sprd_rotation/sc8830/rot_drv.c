...
static int rot_k_check_param(ROT_CFG_T * param_ptr)
{
	...
	if ((param_ptr->src_addr.y_addr & ALGIN_FOUR)
		...
		printk("rot_k check param error src y=0x%x,u=0x%x,v=0x%x, dst y=0x%x,u=0x%x,v=0x%x\n",
		param_ptr->src_addr.y_addr,
		param_ptr->src_addr.u_addr,
		param_ptr->src_addr.v_addr,
		param_ptr->dst_addr.y_addr,
		param_ptr->dst_addr.u_addr,
		param_ptr->dst_addr.v_addr);
		return -1;
	}
