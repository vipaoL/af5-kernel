...
#define GSENSOR_CAL_FILE_PATH			"/productinfo/lis3dh_cal"       //kxtj Calbration file path
#define GSENSOR_CALI_PROC_FILE			"kxtj_cali"
#endif
...
#ifdef GSENSOR_CALI_SUPPORT
static struct proc_dir_entry *gsensor_cali_proc = NULL;
static int accel_cali[3];
#endif
...
static int lis3dh_acc_enable(struct lis3dh_acc_data *acc)
{
	...
	{
#ifdef GSENSOR_CALI_SUPPORT
		if(lis3dh_acc_misc_data->get_cali_flag == 0)
		{
			read_factory_calibration(accel_cali);
			lis3dh_acc_misc_data->get_cali_flag = 1;
		}
#endif
...
static int gsensor_acc_cali_data(int sample_cnt, int *buf)
{
	...
	if((lis3dh_acc_misc_data==NULL) || (buf==NULL) || (sample_cnt==0))
	{
		return -1;
	}
	...
	{
		if(lis3dh_acc_get_acceleration_data(lis3dh_acc_misc_data, &xyz_data[0]) < 0)
		{
			return -1;
		}
...
static int gsensor_cali_write_proc(struct file *file, const char *buffer, size_t count, loff_t *ppos)
{
...
static int gsensor_cali_read_proc(struct file *file, char *buffer, size_t count, loff_t *ppos)
{
	int *cali_ptr = accel_cali;
...
static const struct file_operations gsensor_cali_proc_fops =
{ 
	.write = gsensor_cali_write_proc,
	.read = gsensor_cali_read_proc
};
#endif
			    ...
			    const struct i2c_device_id *id)
{
	...
	*/
	...
	{
		acc->client = NULL;
		printk("%s I2C driver not registered!Device unknown %d\n", LIS3DH_ACC_DEV_NAME, tempvalue);
		goto err_i2c_test;
	}
...
*/
	...
	lis3dh_acc_misc_data->get_cali_flag = 0;
	gsensor_cali_proc = proc_create(GSENSOR_CALI_PROC_FILE, 0660, NULL, &gsensor_cali_proc_fops);
	if (gsensor_cali_proc == NULL)
	{
		printk("create_proc_entry %s failed\n", GSENSOR_CALI_PROC_FILE);
	}
#endif
