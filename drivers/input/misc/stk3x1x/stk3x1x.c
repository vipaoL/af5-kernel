...
static int PS_MAX_XTALK = 1500;//500;
#endif
...
static struct stk3x1x_data *stk3x1x_i2c_data = NULL;
...
{
        return sprintf(buf,"%s",STK3X1X_DEV_NAME);
}
...
static int read_factory_calibration(struct stk3x1x_data *pstk_data)
{
	...
	fp = filp_open(PS_CAL_FILE_PATH, O_RDONLY, (S_IRUSR|S_IRGRP));
	if (IS_ERR(fp))
	{
...
static struct device_attribute als_lux_attribute = __ATTR(lux,0666,stk_als_lux_show,stk_als_lux_store);
...
#else	
	err = request_any_context_irq(irq, stk_oss_irq_handler, IRQF_TRIGGER_LOW, STK3X1X_DEV_NAME, ps_data);
#endif	
	if (err < 0) 
	{
...
#ifdef CONFIG_HAS_EARLYSUSPEND
static void stk3x1x_early_suspend(struct early_suspend *handler)
{
	stk3x1x_suspend(&stk3x1x_i2c_data->client->dev);
}

static void stk3x1x_late_resume(struct early_suspend *handler)
{
	stk3x1x_resume(&stk3x1x_i2c_data->client->dev);
}
...
static int stk3x1x_set_input_devices(struct stk3x1x_data *ps_data)
{
	...
	ps_data->ps_input_dev->phys = STK3X1X_INPUT_DEV;
	...
	__set_bit(EV_ABS, ps_data->ps_input_dev->evbit);
	input_set_abs_params(ps_data->ps_input_dev, ABS_DISTANCE, 0, 1, 0, 0);
...
//wangyang add
static struct stk3x1x_platform_data stk3x1x_pls_info={
.state_reg = 0x0,
/* disable all */
...
static long stk3x1x_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
        ...
        printk("cmd = %d, %d\n", _IOC_NR(cmd), cmd);
        switch (cmd) {
        case STK_IOCTL_SET_PFLAG: {
                if (copy_from_user(&flag, argp, sizeof(flag)))
                        return -EFAULT;
                printk("STK_IOCTL_SET_PFLAG = %d\n", flag);
                if (1 == flag) {
                        if (stk3x1x_enable_ps(stk3x1x_i2c_data, 1, 0))
                                return -EIO;
                } else if (0 == flag) {
                        if (stk3x1x_enable_ps(stk3x1x_i2c_data, 0, 0))
                                return -EIO;
                } else {
                        return -EINVAL;
                }
        ...
        break;
        case STK_IOCTL_SET_LFLAG: {
                if (copy_from_user(&flag, argp, sizeof(flag)))
                        return -EFAULT;
                printk("STK_IOCTL_SET_LFLAG = %d\n", flag);
                if (1 == flag) {
                        if (stk3x1x_enable_als(stk3x1x_i2c_data, 1))
                                return -EIO;
                } else if (0 == flag) {
                        if (stk3x1x_enable_als(stk3x1x_i2c_data, 0))
                                return -EIO;
                } else {
                        return -EINVAL;
                }
        ...
        break;
        case STK_IOCTL_GET_PFLAG: {
                flag = p_flag;
                printk("STK_IOCTL_GET_PFLAG = %d\n", flag);
                if (copy_to_user(argp, &flag, sizeof(flag)))
                        return -EFAULT;
        }
        break;
        case STK_IOCTL_GET_LFLAG: {
                flag = l_flag;
                printk("STK_IOCTL_GET_LFLAG = %d\n", flag);
                if (copy_to_user(argp, &flag, sizeof(flag)))
                        return -EFAULT;
        }
        break;
        default:
                printk("unknown command: 0x%08X  (%d)\n", cmd, cmd);
                break;
        }
        return 0;
}
...
static ssize_t alsps_calibration_read(struct file *f, char __user *buf, size_t count, loff_t *ppos)
{
	...
	data_buf[0] = ps_value;
	data_buf[1] = stk3x1x_i2c_data->ps_thd_h;
	data_buf[2] = stk3x1x_i2c_data->ps_thd_l;
                        ...
                        const struct i2c_device_id *id)
{
	...
	client->dev.platform_data=&stk3x1x_pls_info;
	i2c_set_clientdata(client,ps_data);

	err = stk3x1x_check_pid(ps_data);
	if(err < 0)
	{
		printk(KERN_ERR "%s: check_pid fail (%d)", __func__, err);
		kfree(ps_data);
		return err;	
	}
...
#endif
	{
		...
		if (np)
		{
			...
			{
				dev_err(&client->dev, "fail to get int_pin\n");
	...
	if(err < 0)
        ...
        if (err) {
                printk("stk3x1x misc_register failed!\n");
                goto err_stk3x1x_misc_register;
        }
...
#ifdef CONFIG_HAS_EARLYSUSPEND
        unregister_early_suspend(&stk3x1x_i2c_data->stk_early_suspend);
#endif
...
#if 0
static struct i2c_board_info  stk3x1x_i2c_boardinfo = {
        I2C_BOARD_INFO("stk3x1x_pls", 0x48),
        .platform_data = &stk3x1x_pls_info,
};
...
static int __init stk3x1x_init(void)
{
	...
	/* ret_i2c = i2c_static_add_device(&stk3x1x_i2c_boardinfo);
	if (ret_i2c < 0) {
