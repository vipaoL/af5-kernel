...
static int tpd_enable_ps(int enable)
{
	...
	//	static struct i2c_client *this_client = NULL;
		unsigned char ps_store_data[4],pbt_buf,ps_store_data1[3];
		int err = 0;
	
		if (enable)
		{
			//printk("CCS+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++msg21xx_proximity_set_enable ,enable=%x \r\n",enable);
			ps_store_data[0] = 0x52;
			ps_store_data[1] = 0x00;  //01	
			ps_store_data[2] = 0x4a;  //24
		...
		err= i2c_master_send(msg21xx_i2c_client, &ps_store_data[0], 4);
	msleep(10);
			ps_store_data1[0] = 0x53;
			ps_store_data1[1] = 0x00;  //01
			ps_store_data1[2] = 0x4a;  //24
			msleep(10);
		err = i2c_master_send(msg21xx_i2c_client, &ps_store_data1[0], 3);
		msleep(10);
		err = i2c_master_recv(msg21xx_i2c_client, &pbt_buf, 1);
		msleep(10);
		//printk("CCS++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++msg21xx_proximity_set_enable ,this_client->addr=%x \r\n",this_client->addr);
		//printk("CCS++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++msg21xx_proximity_set_enable ,pbt_buf=%x \r\n",pbt_buf);
...
static char keymap[]={KEY_MENU,KEY_HOMEPAGE,KEY_BACK};
...
#define TP_READ_FACTORY_VER

#ifdef TP_READ_FACTORY_VER
static ssize_t tpd_info_readVersion(struct file *f, char __user *buf, size_t count, loff_t *ppos)
{
	 int data_buf[3]={0};
	 int ret = 0;
	 int time = 0;
	 unsigned short major = 0xff, minor = 0xff;	
	  unsigned char dbbus_tx_data[3];
	 unsigned char dbbus_rx_data[4] ;
	 ...
	 while(time < 3)
	 {
		...
		dbbus_tx_data[1] = 0x00;
		dbbus_tx_data[2] = 0x2a;
		i2c_master_send(msg21xx_i2c_client, &dbbus_tx_data[0], 3);
		msleep(20);
		ret = i2c_master_recv(msg21xx_i2c_client, &dbbus_rx_data[0], 4);
		printk("%s, dbbus_rx_data = [0x%x] [0x%x] [0x%x] [0x%x]", __func__, dbbus_rx_data[0], dbbus_rx_data[1], dbbus_rx_data[2], dbbus_rx_data[3]);
	 ...
	 data_buf[0] = minor;
	 data_buf[1] = major;
	 printk("***FW Version tpd_info_read = %d.%d***\n", data_buf[0],data_buf[1]); 
...
static const struct file_operations tpd_info_fops = {
	...
	.read =  tpd_info_readVersion,
};
static struct miscdevice tpd_info_device = {
	.minor = MISC_DYNAMIC_MINOR,
	.name = "tp_ver",
	.fops = &tpd_info_fops,
};
...
static int sy_rxdata(struct touch_info *cinfo, struct touch_info *pinfo)
	...
	if((temp_checksum==reg_val[7])&&(reg_val[0] == 0x52))
	{
		...
		if (tpd_proximity_flag == 1)
		{
			...
			{
				if (reg_val[5] == 0x80)   //near 
				{
					tpd_proximity_detect = 0;	
				}
				else if(reg_val[5] == 0x40)
				{
					tpd_proximity_detect = 1;//far
				}
...
#endif
#ifdef CONFIG_OF
static struct msg2138_ts_platform_data *msg2138_ts_parse_dt(struct device *dev)
{
...
#endif
static int msg2138_test_i2c(struct i2c_client *client)
{
	unsigned char reg_val[8] = {0};
	int ret;

	ret = i2c_master_recv(client, reg_val, 8);
	printk("msg2138_test_i2c: %d\r\n", ret);
					 ...
					 const struct i2c_device_id *id)
{
	...
	/*if (0 != tp_cg_flag)
	{
		...
		if (np && !pdata){
			pdata = msg2138_ts_parse_dt(&client->dev);
			if(pdata){
...
#endif
	error=msg2138_test_i2c(client); 
	printk("pixcir_i2c_ts_probe error=%d \n",error);
	if(error<0)
	{
		printk("pixcir_i2c_ts_probe not detect msg2138 device\r\n");
...
/*#ifdef CONFIG_HAS_EARLYSUSPEND
	...
	/*if(pixcir_config_intmode()<0) {
		...
		printk("msg2138  proximity err=%d \n",err);
		if (err)
		{
			printk("%s: msg2138 register failed err = %d \n", __func__,err);
		}
		focaltech_pls_input_dev = input_allocate_device();
		if (!focaltech_pls_input_dev) 
		{
			printk("%s: msg2138 input allocate device failed\n", __func__);
		}
		else
		{
			...
			if (err < 0)
			{
				printk("%s: input device regist failed err = %d \n", __func__,err);
			}
		}
#endif
...
#ifdef TP_READ_FACTORY_VER
		printk("msg2138  tp_read factory ver !\n");
		if (misc_register(&tpd_info_device) < 0)
		{
			printk("pixcir_i2c_ts_probe: tpd_info_device register failed\n");
		}
#endif
