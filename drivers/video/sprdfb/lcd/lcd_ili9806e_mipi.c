...
static LCM_Init_Code init_data[] = {
    ...
    {LCM_SEND(2), {0x40, 0x11}},                 // output SDA
    {LCM_SEND(2), {0x41, 0x22}},                 // DE = 1 Active
    {LCM_SEND(2), {0x42, 0x02}},                 // 480 X 800
    {LCM_SEND(2), {0x43, 0x89}},                 // 2-Dot Inversion
    {LCM_SEND(2), {0x44, 0x87}},                 // output SDA
    ...
    {LCM_SEND(2), {0x53, 0x73}},                 // 2-Dot Inversion
    {LCM_SEND(2), {0x56, 0x00}},
    {LCM_SEND(2), {0x57, 0x50}},                 // 480 X 800


    {LCM_SEND(2), {0x60, 0x05}},                 // SDTI
...
static int32_t ili9806e_enter_sleep(struct panel_spec *self, uint8_t is_sleep)
{
	...
	if(is_sleep){
		...
		printk("ili9806e_mipi_init out(20141218)\n");
		self->ops->panel_reset(self);
		ili9806e_mipi_init(self);
	}
...
static uint32_t ili9806e_readpowermode(struct panel_spec *self)
{
	...
	printk("lcd_ili9806e mipi read power mode fail!0x0A value is 0x%x! , read result(%d)\n", read_data[0], read_rtn);
	mipi_eotp_set(1,1);
	return 0x0;
}
