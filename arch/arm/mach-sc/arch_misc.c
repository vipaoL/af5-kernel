...
atomic_t couter4pmu_apb_sleep_ctrl = ATOMIC_INIT(0);
...
void sci_ctrl_pmu_sleep_ctrl(u32 flag)
{
	if(1 == flag) {
		sci_glb_set(REG_PMU_APB_SLEEP_CTRL,BIT_AP_SLEEP_XTL_ON);
		atomic_inc(&couter4pmu_apb_sleep_ctrl);
	} else {
		if(atomic_dec_and_test(&couter4pmu_apb_sleep_ctrl)) {
...
int sci_get_couter4pmu_apb_sleep_ctrl()
{
	return atomic_read(&couter4pmu_apb_sleep_ctrl);
}

EXPORT_SYMBOL(sci_ctrl_pmu_sleep_ctrl);
EXPORT_SYMBOL(sci_get_couter4pmu_apb_sleep_ctrl);
